...
export function formatPrice(priceStr: string): string {
  ...
  return cleanedPrice.replace(/(\d+(?:\.\d{3})+|\d{4,})/g, (match) => {
    ...
    if (num >= 1000000) {
      ...
      return millions % 1 === 0 ? `${millions} Juta` : `${millions.toFixed(1).replace(".", ",")} Juta`;
    } else if (num >= 1000) {
...
function AddOnAccordion({ addons, isDark = false }: { addons?: AddOn[], isDark?: boolean }) {
  const [isOpen, setIsOpen] = useState(false);

  if (!addons || addons.length === 0) return null;

  return (
    <div className={`mt-4 border-t pt-4  ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full cursor-pointer flex items-center justify-between text-left group"
      >
        <span className={`font-semibold text-sm transition-colors    ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
          ADD-ONS
        </span>
        <ChevronDown
          className={`w-4 h-4 transition-transform duration-300 ${isDark ? 'text-gray-400' : 'text-gray-500'} ${isOpen ? "rotate-180" : ""
            }`}
        />
      </button>
          ...
          {addons.map((addon, idx) => (
            <div key={idx} className="text-sm">
              <div className="flex justify-between items-start gap-4">
                <span className={`font-medium ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>+ {addon.name}</span>
                <span className={`font-semibold shrink-0`}>
                  {formatPrice(addon.price)}
                </span>
              </div>
              <p className={`text-xs mt-1 hidden sm:block ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                {addon.description}
              </p>
            </div>
          ))}
  ...
  return (
    ...
    >
      ...
      <div className=" mx-auto relative z-10 w-full">
        ...
        {/* Tab Navigation */}
        <div ref={wrapperRef} className="w-full h-[62px] md:h-auto mb-12 relative flex justify-center">
          <div
            ...
            <div className="bg-white p-1 rounded-full border border-gray-400 flex w-full max-w-[95%] sm:max-w-lg mx-auto">
              {TABS.map((tab) => (
                <button
                  ...
                  className={`flex-1 text-center py-2 px-1 xs:px-3 sm:px-6 rounded-full text-xs xs:text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === tab
                    ? "bg-[var(--color-lime)] text-gray-900 "
              ...
              >
                ...
                {(plan.badge || isPopular) && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
                    <span className="bg-[var(--color-lime)] px-4 py-1.5 rounded-full shadow-sm flex items-center gap-1.5">
                      {isPopular && !isEnterprise && <Star className="w-3 h-3 fill-current text-black-100 shrink-0" />}
                ...
                <div className="relative z-10 flex-1">
                  ...
                  >
                    <Icon
                      className={`w-7 h-7 ${isPopular && !isEnterprise ? "text-[var(--color-lime)]" : isEnterprise ? "text-[var(--color-lime)]" : ""} `}
                    />
                  </div>

                  <div className="mb-6">
                    <h3
                      className={`text-2xl font-bold mb-2 ${isPopular && !isEnterprise ? "text-[var(--color-lime)]" : ""
                        }`}
                    >
                      {plan.name}
                    </h3>
                        ...
                        >
                          {isModul ? (
                            ...
                            <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                              <Check
                                className={`h-3 w-3 ${isPopular && !isEnterprise ? "text-[var(--color-lime)]" : isEnterprise ? "text-[var(--color-lime)]" : "text-primary"}`}
                                strokeWidth={3}
                              />
                            </div>
                          )}
                          <span
                            className={`text-sm leading-relaxed ${isPopular && !isEnterprise
                              ? "text-[var(--color-lime)]"
                              : isEnterprise
                                ? isModul ? "text-gray-400" : "text-gray-300"
                                : "text-gray-800"
                              }`}
                          >
                            {displayText}
                ...
                <div className="mt-auto relative z-20">
                  ...
                  <div className={isPopular && !isEnterprise ? "text-[var(--color-lime)]" : ""}>
                    <AddOnAccordion addons={plan.add_ons} isDark={isPopular || isEnterprise} />
                  </div>
                  ...
                  <Button
                    className={`
                      ...
                      ${(isPopular || isEnterprise)
                        ? "bg-[var(--color-lime)] text-black-100 hover:bg-[var(--color-lime)]/90"
                        : "bg-black-100 text-[var(--color-lime)] hover:bg-black-100/90"
                      }
                    `}
                    onClick={() => sendWhatsapp(plan.name, isEnterprise)}
                  >
                    <span className="font-semibold">{isEnterprise ? "Konsultasi Gratis" : "Pilih Paket"}</span>
                  </Button>
                </div>
              </Card>
            );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch mt-12 max-w-5xl mx-auto w-full">
          ...
          <div className="bg-white p-8 rounded-2xl border border-gray-600/70 flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center gap-2.5 mb-8 border-b border-gray-100 pb-4">
                ...
                <h4 className="text-lg font-bold text-amber-600">Informasi Penting</h4>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="space-y-1.5">
                  ...
                  <p className="text-gray-500 leading-relaxed text-xs pl-3.5">Harga belum termasuk konten (teks, logo, dan foto produk) dari klien.</p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-gray-500 leading-relaxed text-xs pl-3.5">Revisi di luar ketentuan paket dikenakan biaya tambahan.</p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-gray-500 leading-relaxed text-xs pl-3.5">Sistem pembayaran 50% DP di muka, 50% setelah proyek selesai.</p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-gray-500 leading-relaxed text-xs pl-3.5">Semua paket sudah termasuk sesi konsultasi gratis.</p>
                </div>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <p className="text-gray-500 leading-relaxed text-xs pl-3.5">Harga final dapat berbeda tergantung kompleksitas aktual hasil sesi konsultasi.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Bottom CTA */}
          <div
            className={` flex flex-col justify-center md:justify-end items-center md:items-end text-center transition-all duration-1000 delay-700 h-full ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              }`}
          >
            ...
            <Button
              ...
              className="group hover:border-[var(--color-lime)] w-full rounded-full md:h-20 h-16 bg-black-100 border-transparent transition-all duration-300 flex items-center justify-center max-w-md"
            >
              <span className="group-hover:scale-105 transition-transform duration-300 text-[var(--color-lime)] px-4 sm:px-8 flex items-center justify-center gap-3 text-md md:text-xl font-bold">
